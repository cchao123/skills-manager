    ...
    <div className={`fixed bottom-6 right-6 flex flex-col gap-2 z-[200] transition-transform duration-300 ${(isDrawerOpen || isScrolling) ? 'translate-x-[calc(100%+1.5rem)]' : ''}`}>
      <div className="relative group">
        <button
          ref={helpButtonRef}
          onClick={onHelpClick}
          className="w-9 h-9 rounded-full bg-white dark:bg-dark-bg-card border border-[var(--border-color)] dark:border-dark-border shadow-[0_18px_34px_-26px_rgba(39,50,72,0.9)] flex items-center justify-center hover:bg-[var(--shell-surface-soft)] dark:hover:bg-gray-700 hover:shadow-[0_24px_40px_-26px_rgba(39,50,72,0.95)] transition-all"
        >
          <Icon name="help" className="text-base text-[var(--text-secondary)] dark:text-gray-400" />
        </button>
      ...
      <div className="relative group">
        <button
          ...
          className="w-9 h-9 rounded-full bg-white dark:bg-dark-bg-card border border-[var(--border-color)] dark:border-dark-border shadow-[0_18px_34px_-26px_rgba(39,50,72,0.9)] flex items-center justify-center hover:bg-[var(--shell-surface-soft)] dark:hover:bg-gray-700 hover:shadow-[0_24px_40px_-26px_rgba(39,50,72,0.95)] transition-all"
        >
          <Icon name="history" className="text-base text-[var(--text-secondary)] dark:text-gray-400" />
        </button>
        <div className="absolute right-full mr-2 top-1/2 -translate-y-1/2 z-50 pointer-events-none hidden group-hover:block">
          <div className="whitespace-nowrap rounded-lg bg-slate-800 dark:bg-slate-700 text-white text-xs font-medium px-2.5 py-1 shadow-lg">
