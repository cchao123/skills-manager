...
export const SkillImportModal: React.FC<SkillImportModalProps> = ({
  ...
  // 添加到导入列表
  const addSelectedToImport = () => {
    const skillsToAdd = sourceAgentSkills.filter(s =>
      selectedSkills.has(s.id) &&
      !existingSkillIds.has(s.id) &&
      !importListSkillIds.has(s.id)
    );
    setImportList(prev => {
      // 去重（按 ID）
      const existingIds = new Set(prev.map(s => s.id));
      const newSkills = skillsToAdd.filter(s => !existingIds.has(s.id));
  ...
  return (
    <div
      className="fixed inset-0 z-[9999] bg-black/10 backdrop-blur-[2px] flex items-center justify-center p-4"
      onClick={onClose}
    >
      <div
        className="bg-white dark:bg-dark-bg-card rounded-2xl shadow-2xl w-full max-w-6xl h-[80vh] overflow-hidden flex flex-col"
      ...
      >
          ...
          <div className="flex-1 flex flex-col border-r border-gray-200 dark:border-dark-border">
            <div className="p-4 border-b border-gray-200 dark:border-dark-border flex items-center justify-between">
              ...
              {selectedSourceAgent && sourceAgentSkills.length > 0 && (
                <button
                  onClick={() => {
                    // 获取可选的技能（排除已存在和已在导入列表中的）
                    const availableSkills = sourceAgentSkills.filter(s =>
                      !existingSkillIds.has(s.id) && !importListSkillIds.has(s.id)
                    );
                    // 检查是否所有可选技能都已选中
                    const isAllAvailableSelected = availableSkills.every(s => selectedSkills.has(s.id));

                    if (isAllAvailableSelected) {
                      setSelectedSkills(new Set());
                    } else {
                      setSelectedSkills(new Set(availableSkills.map(s => s.id)));
                    }
                  }}
                ...
                >
                  {(() => {
                    ...
                    const isAllAvailableSelected = availableSkills.length > 0 && availableSkills.every(s => selectedSkills.has(s.id));
                    return isAllAvailableSelected ? '取消全选' : '全选';
                  })()}
                </button>
              )}
            </div>
              ...
              {!selectedSourceAgent ? (
                ...
                <div className="space-y-2">
                  {sourceAgentSkills.map(skill => {
                    ...
                    const isDisabled = isExisting || isInImportList;
                      ...
                      <div
                        ...
                        className={`flex items-start gap-2 p-3 rounded-lg border transition-colors ${
                          isExisting
                            ...
                            : isInImportList
                              ? 'border-[#b71422] dark:border-rose-600 bg-rose-50 dark:bg-rose-900/30 opacity-70 cursor-not-allowed'
                              : selectedSkills.has(skill.id)
                                ? 'border-amber-400 dark:border-amber-600 bg-amber-50 dark:bg-amber-900/20 shadow-sm cursor-pointer'
                        ...
                        onClick={() => {
                          if (isDisabled) return; // 已存在或已在导入列表的技能不允许点击
                      ...
                      >
                        <div className="w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 mt-0.5">
                          {isExisting ? (
                            ...
                            <div className="w-2.5 h-2.5 rounded-sm bg-[#b71422]" />
                          ) : selectedSkills.has(skill.id) ? (
            ...
            <div className="p-4 border-b border-gray-200 dark:border-dark-border flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                {selectedSkills.size > 0 && (
                  <button
                    onClick={addSelectedToImport}
                    className="px-3 py-1.5 rounded-lg text-xs font-medium bg-[#b71422] hover:bg-[#a01220] text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    disabled={selectedSkills.size === 0}
                  >
                    添加选中 ({selectedSkills.size})
                  </button>
                )}
                {importList.length > 0 && (
                  <button
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {importList.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-slate-400 dark:text-gray-500">
                  ...
                  <p className="text-xs mt-1">请先选择 Agent 和技能，然后点击"添加选中"</p>
                </div>
              ) : (
                <div className="space-y-2">
        ...
        {/* Footer */}
        <div className="flex items-center justify-end p-4 border-t border-gray-200 dark:border-dark-border gap-3">
          <button
            onClick={onClose}
