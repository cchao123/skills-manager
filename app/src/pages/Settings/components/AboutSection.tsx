...
export const AboutSection: React.FC = () => {
  ...
  switch (status) {
    ...
    case 'update-available':
      ...
      statusTone = 'text-[var(--accent-primary)] dark:text-red-400';
      break;
    case 'up-to-date':
  ...
  return (
    <div className="flex-1 bg-[var(--bg-card)] dark:bg-dark-bg-card rounded-2xl shadow-sm border border-[var(--border-color)] dark:border-dark-border flex flex-col justify-center">
      <div className="flex flex-col items-center text-center">
        ...
        <div className="w-full max-w-md mb-6">
          ...
          <div className="mt-2 flex items-center justify-center gap-2 text-xs">
            ...
            <button
              ...
              disabled={checking}
              className="text-slate-500 dark:text-gray-400 hover:text-[var(--accent-primary)] dark:hover:text-red-400 transition-colors disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center gap-1"
          ...
          {status === 'update-available' && releaseUrl && (
            <div className="mt-3 flex justify-center">
              <button
                ...
                className="px-4 py-2 bg-[var(--accent-primary)] hover:bg-[var(--accent-primary-strong)] text-white rounded-lg text-xs font-bold shadow-sm transition-all flex items-center gap-1.5"
              >
          ...
          {(status === 'no-release' || status === 'error') && (
            <div className="mt-3 flex justify-center">
              <button
                ...
                className="px-4 py-2 bg-[var(--shell-surface-soft)] hover:bg-[var(--shell-surface-muted)] dark:bg-dark-bg-tertiary dark:hover:bg-dark-bg-card text-[var(--text-primary)] dark:text-white rounded-lg text-xs font-bold transition-all flex items-center gap-1.5"
              >
        ...
        <div className="w-16 h-1 bg-gradient-to-r from-[var(--accent-primary)] to-[var(--accent-primary-strong)] rounded-full mb-6"></div>
        <div className="text-sm text-slate-600 dark:text-gray-300 space-y-2">
          <ul className="list-disc pl-5 text-left space-y-2 marker:text-[var(--accent-primary)] marker:font-bold">
            {aboutHighlights.map((item) => (
              <li key={item.title}>
                <span className="text-[var(--accent-primary)] font-bold">{item.title}</span>：{item.description}
              </li>
            ))}
          </ul>
        </div>
